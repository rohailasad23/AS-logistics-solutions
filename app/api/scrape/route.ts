...
export async function GET(req:NextRequest){
  ...
  if(dot){
    ...
    let res:Response; try{res=await fetch(url,{headers:{"user-agent":"Safer Scraber/1.0 (low-rate carrier research tool)",accept:"text/html"},redirect:"follow"});}catch{return NextResponse.json({error:"FMCSA (AI) could not be reached"},{status:502});}
  ...
  let res:Response; try{res=await fetch(url,{headers:{"user-agent":"Safer Scraber/1.0 (low-rate carrier research tool)",accept:"text/html"},redirect:"follow"});}catch{return NextResponse.json({error:"FMCSA could not be reached"},{status:502});}
